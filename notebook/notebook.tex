...
Complexity: O(\textbackslash\{\}log(\textbackslash\{\}text\{scale\}) \textbackslash\{\}cdot 100 \textbackslash\{\}cdot 9) \textbackslash\{\}approx O(1) with a small constant.\\
Limitations:\\
...
Author: KACTL / botellot / ICPC BRO\\
Description: Approximates the definite integral \$\textbackslash\{\}int\_\{a\}\textasciicircum{}\{b\} f(x) dx\$ (area under the curve)\\
...
Usage:\\
double area = quad(0, M\_PI, [](double x) \{ return sin(x); \}); // Returns 2.0\\
\end{flushleft}
